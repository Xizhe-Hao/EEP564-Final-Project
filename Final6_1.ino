...
void loop()
{
    ...
    while(stop_inferencing == false) {
        ...
        ei_printf("Loop iteration - Game mode: %s, Params set: %s\n", 
                  game_mode ? "ON" : "OFF", game_params_set ? "YES" : "NO");
        ...
        for (uint16_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
            ei_printf("  %s: %.5f", ei_classifier_inferencing_categories[i], result.classification[i].value);
        ...
        ei_printf("Debug: Highest confidence class index: %d, confidence: %.3f\n", userHand, maxConfidence);
        
        // Process game result if in game mode
        if (game_mode && game_params_set) {
            ei_printf("\n=== GAME PROCESSING ===\n");
            
            // Validate userHand index
            if (userHand >= 0 && userHand < 3) {
                ei_printf("Detected gesture: %s (%.1f%% confidence)\n", 
                          hand_names[userHand], maxConfidence * 100);
            } else {
                ei_printf("Error: Invalid hand gesture index %d\n", userHand);
                userHand = 0; // Default to Rock if invalid
            }
            
            // Check confidence threshold
            if (maxConfidence < 0.3) {
                ei_printf("Confidence too low (%.1f%%)! Please try again.\n", maxConfidence * 100);
                ei_printf("Make sure your hand gesture is clear and well-lit.\n");
                ei_printf("Minimum confidence required: 30%%\n");
            } else {
                if (maxConfidence < 0.6) {
                    ei_printf("Warning: Low confidence (%.1f%%). Result may be inaccurate.\n", maxConfidence * 100);
                }
                
                ei_printf("Debug: Calling determineWinner(%d, %d, %d)\n", userHand, systemHand, ruleConfig);
