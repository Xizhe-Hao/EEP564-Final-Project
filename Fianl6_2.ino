...
void loop() {
    ...
    static int ruleConfig = 0;          // Store rule configuration for each game round
    static int systemHand = 0;          // Store system’s chosen hand
    static bool game_params_set = false; // Indicates whether ruleConfig & systemHand are assigned
    static bool warmed_up = false;      // Indicates whether camera/model warm-up is done

    while (!stop_inferencing) {
        // 1) Check for serial input from user
        if (ei_get_serial_available() > 0) {
            char cmd = ei_get_serial_byte();
            ei_printf("Received command: '%c'\n", cmd);

            if (cmd == 'b' || cmd == 'B') {
                // If user presses 'b', exit the main loop
                ei_printf("Inferencing stopped by user\r\n");
                stop_inferencing = true;
                continue;
            }
            else if (cmd == 'g' || cmd == 'G') {
                // If user presses 'g', activate game mode
        ...
        // 2) Branch: if game_mode is active, run the enhanced game logic
        if (game_mode) {
            // 2.1) Warm-up camera and model exactly once per game session
            if (!warmed_up) {
                // Determine resize dimensions
                uint32_t warm_resize_cols, warm_resize_rows;
                bool warm_do_resize = false;
                int warm_res = calculate_resize_dimensions(
                ...
                if (warm_res) {
                    ei_printf("ERR: Failed to calculate resize dimensions during warm-up (%d)\r\n", warm_res);
                    break;
                }

                // Allocate a small buffer for warm-up frames
                void *warm_mem = ei_malloc(warm_resize_cols * warm_resize_rows * 2);
                if (warm_mem == NULL) {
                    ei_printf("ERR: Failed to allocate warm-up buffer\r\n");
                    break;
                }
                uint8_t *warm_buf = (uint8_t *)DWORD_ALIGN_PTR((uintptr_t)warm_mem);

                // Initialize camera if not already initialized
                if (!is_initialised) {
                    if (!ei_camera_init()) {
                        ei_printf("ERR: Failed to initialize camera for warm-up\r\n");
                        ei_free(warm_mem);
                        break;
                    }
                }

                ei_printf("Warming up camera and model...\n");
                for (int i = 0; i < 2; i++) {
                    // Capture and discard two frames to stabilize auto-exposure and model caches
                    ei_camera_capture(EI_CLASSIFIER_INPUT_WIDTH, EI_CLASSIFIER_INPUT_HEIGHT, warm_buf);

                    ei::signal_t warm_signal;
                    warm_signal.total_length = EI_CLASSIFIER_INPUT_WIDTH * EI_CLASSIFIER_INPUT_HEIGHT;
                    warm_signal.get_data = &ei_camera_cutout_get_data;

                    ei_impulse_result_t warm_result;
                    run_classifier(&warm_signal, &warm_result, false);

                    ei_sleep(200);  // Brief pause between warm-up frames
                }
                warmed_up = true;
                ei_printf("Warm-up complete.\n");
                ei_free(warm_mem);
            }

            // 2.2) Generate game parameters (random rule + system hand) once per round
            if (!game_params_set) {
                ei_printf("\n--- Starting Rock-Paper-Scissors Round ---\n");
                ...
                // Extra 0.5-second delay to allow user’s hand to stabilize
                ei_sleep(500);
            }

            // 2.3) Perform 3-frame voting to determine final user gesture
            if (game_params_set) {
                const int VOTE_FRAMES = 3;
                int vote_count[3] = {0, 0, 0};

                // Determine resize dimensions for voting frames
                uint32_t resize_cols, resize_rows;
                bool do_resize = false;
                int res_dim = calculate_resize_dimensions(
                    EI_CLASSIFIER_INPUT_WIDTH,
                    EI_CLASSIFIER_INPUT_HEIGHT,
                    &resize_cols,
                    &resize_rows,
                ...
                if (res_dim) {
                    ei_printf("ERR: Failed to calculate resize dimensions for voting (%d)\r\n", res_dim);
                    break;
                }

                // Allocate buffer for snapshot frames
                void *snapshot_mem = ei_malloc(resize_cols * resize_rows * 2);
                ...
                ei_printf("\nCapturing and classifying %d frames...\n", VOTE_FRAMES);
                for (int f = 0; f < VOTE_FRAMES; f++) {
                    ei_printf(" Frame %d/%d:\n", f + 1, VOTE_FRAMES);

                    // Capture one frame
                    ei_camera_capture(EI_CLASSIFIER_INPUT_WIDTH, EI_CLASSIFIER_INPUT_HEIGHT, snapshot_buf);
                    ...
                    if (ei_err != EI_IMPULSE_OK) {
                        ei_printf(" ERR: Inference error (%d)\n", ei_err);
                        continue;  // Skip counting this frame
                    }
                    ...
                    ei_printf("   Detected: %s (%.1f%%)\n",
                              hand_names[best_class], best_conf * 100.0f);

                    // Only count if confidence ≥ 0.40
                    if (best_conf >= 0.40f) {
                        vote_count[best_class]++;
                        ei_printf("   -> Counted for voting.\n");
                    } else {
                        ei_printf("   -> Confidence too low, ignored.\n");
                    }

                    ei_sleep(200);
                }

                // Determine final userHand by majority vote
                int userHand = 0;
                int max_votes = vote_count[0];
                for (int i = 1; i < 3; i++) {
                ...
                // If no frame passed confidence threshold, ask user to retry
                if (max_votes == 0) {
                    ei_printf("\nNo confident detection. Please try again!\n");
                    ei_free(snapshot_mem);
                    game_params_set = false;
                    continue;  // Return to waiting for next command
                }

                ei_printf("\n=== Voting complete: Final gesture = %s (votes: %d) ===\n",
                          hand_names[userHand], max_votes);
                ...
                game_mode = false;
                game_params_set = false;
                ei_printf("\nPress 'g' to play again, or 'b' to stop inferencing.\n");
                continue;
            }
        }
        // 3) If not in game mode, run continuous inference mode (real-time predictions)
        else {
            ei_printf("\nStarting inferencing in 2 seconds...\n");
            ei_printf("Press 'g' to play Rock-Paper-Scissors, or 'b' to stop\r\n");
            ...
            if (snapshot_mem == NULL) {
                ei_printf("ERR: Failed to allocate snapshot memory\r\n");
                break;
            }
            uint8_t *snapshot_buf = (uint8_t *)DWORD_ALIGN_PTR((uintptr_t)snapshot_mem);
            ...
            // Clean up memory
            ei_free(snapshot_mem);
            ei_printf("End of continuous inference iteration\n\n");
        }
    }

    // 4) Exiting: deinitialize camera if it was initialized
    if (is_initialised) {
        ei_camera_deinit();
        is_initialised = false;
    }

    ei_printf("Exited main loop.\n");
}
