...
int generateRandomRule() {
    return random(8); // Returns 0-7 for different rule configurations
}

int determineWinner(int userHand, int systemHand, int ruleConfig) {
    if (userHand == systemHand) {
        // Always a draw if both chose the same hand
        return 0;
    }
    ...
    switch (ruleConfig) {
        ...
        case 0:
            if ((userHand == ROCK     && systemHand == SCISSORS) ||
                (userHand == ROCK     && systemHand == PAPER)    ||
        ...
        // 1:
        //    Rock > Scissors, Rock > Paper, Paper > Scissors
        case 1:
            if ((userHand == ROCK  && systemHand == SCISSORS) ||
                (userHand == ROCK  && systemHand == PAPER)    ||
        ...
        // 2: 
        //    Scissors > Rock, Scissors > Paper, Rock > Paper
        case 2:
            if ((userHand == SCISSORS && systemHand == ROCK)   ||
                (userHand == SCISSORS && systemHand == PAPER)  ||
                (userHand == ROCK     && systemHand == PAPER)) {
                return 1;
            }
        ...
        // 3: 
        //    Scissors > Rock, Scissors > Paper, Paper > Rock
        case 3:
            if ((userHand == SCISSORS && systemHand == ROCK)  ||
                (userHand == SCISSORS && systemHand == PAPER) ||
        ...
        case 4:
            if ((userHand == PAPER && systemHand == ROCK)     ||
                (userHand == PAPER && systemHand == SCISSORS) ||
                (userHand == ROCK  && systemHand == SCISSORS)) {
                return 1;
            }
            return -1;

        // 5: 
        //    Paper > Rock, Paper > Scissors, Scissors > Rock
        case 5:
            if ((userHand == PAPER    && systemHand == ROCK)   ||
                (userHand == PAPER    && systemHand == SCISSORS) ||
        ...
        // Default: if ruleConfig is out of range, treat as draw
        default:
            return 0;
    }
}
...
void setup() {
    ...
    ei_printf("Press 'g' to play a round\n");
    ei_printf("=================================\n\n");
    ...
    ei_printf("\nReady! Press 'g' to play\n");
    ei_printf("Score - Wins: %d, Losses: %d, Draws: %d\n\n", user_wins, system_wins, draws);
}
...
*/
void loop() {
    // Only process when user presses 'g'
...
void playGame() {
    ...
    int ruleConfig = generateRandomRule();
    int systemHand = generateSystemHand();
    
    // Show the rules for this round
    displayRuleConfig(ruleConfig);
    
    ei_printf("\nGet ready to show your hand!\n");
    ei_printf("3...\n");
    ei_sleep(1000);
    ei_printf("2...\n");
    ei_sleep(1000);
    ...
    int userHand = captureAndClassify();
    ...
    if (userHand >= 0) {
        ...
        ei_printf("Failed to detect your hand. Press 'g' to try again.\n");
    }
}

/**
* @brief      Capture image and classify hand gesture
* @return     Returns detected hand (0-2) or -1 on error
*/
int captureAndClassify() {
    ...
    // Setup resize dimensions
    uint32_t resize_col_sz;
    uint32_t resize_row_sz;
    bool do_resize = false;
    if (calculate_resize_dimensions(EI_CLASSIFIER_INPUT_WIDTH, EI_CLASSIFIER_INPUT_HEIGHT, 
                                   &resize_col_sz, &resize_row_sz, &do_resize) != 0) {
        ei_camera_deinit();
        return -1;
    }
    ...
    if (snapshot_mem == NULL) {
        ei_printf("ERR: Failed to allocate memory\n");
        ei_camera_deinit();
        return -1;
    }
    ...
    ei_printf("\nDetected: %s (%.1f%% confidence)\n", 
              hand_names[detectedHand], maxConfidence * 100);
    
    // Check confidence threshold
    if (maxConfidence < 0.3) {
        ei_printf("Confidence too low! Please try again.\n");
        detectedHand = -1;
    }
